	...
	<main>
		<div class="container">
			...
			while($fila = $consulta->fetch_array(MYSQLI_ASSOC)){
				array_push($arrayId, (int)$fila['idEquipos'], (int)$fila['pais']);
			}
			print_r($arrayId);
			shuffle($arrayId);
			/*
			if(sizeof($arrayId) != 32){
				echo "no se puede sortear";
			}
			else{
				do{
					$query = "SELECT * FROM Equipos";
			...
			while($fila = $consulta->fetch_array(MYSQLI_ASSOC)){
				array_push($arrayId, (int)$fila['idEquipos']);
			}
					$idEquipoA = (int)$arrayId[0];
					$idEquipoB = (int)$arrayId[1];
					$idEquipoC = (int)$arrayId[2];
					$idEquipoD = (int)$arrayId[3];
